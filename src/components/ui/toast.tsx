  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-4 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        ...
        success:
          "success group border-success/30 bg-success/10 text-success-foreground dark:bg-success/15",
        danger:
          "destructive group border-destructive/30 bg-destructive/10 text-destructive dark:bg-destructive/15 dark:text-destructive-foreground",
        warning:
          "warning group border-warning/30 bg-warning/10 text-warning-foreground dark:bg-warning/15",
      },
    },
    defaultVariants: {
      variant: "default",
...
function showToast({
  ...
  const bgMap = {
    ...
    success: "bg-success/10 text-success-foreground border-success/30 dark:bg-success/15",
    danger: "bg-destructive/10 text-destructive border-destructive/30 dark:bg-destructive/15 dark:text-destructive-foreground",
      ...
      >
        ...
        {action && (
          <button
            ...
            className="shrink-0 rounded-md px-2 py-1 text-xs font-medium text-primary hover:bg-primary/10 transition-colors"
          >
            {action.label}
          </button>
        )}
...
const hotToast: ToastAPI = Object.assign(
